...
func (d *DB) StoreRepos(ctx context.Context, orgRepoNames []string) error {
	...
	for _, orn := range orgRepoNames {
		valueParts = append(valueParts, "('"+orn+"')")
	}

	query := fmt.Sprintf(`
INSERT INTO repos (org_repo_name)
VALUES %s
ON CONFLICT (org_repo_name) DO NOTHING;`, strings.Join(valueParts, ",\n\t"))
...
func (d *DB) StoreRepoTags(ctx context.Context, repoTags []*RepoTag) error {
	...
	var repoTagsValueParts []string
	var orgRepoNamesConditionals string
	orgRepoNames := make(map[string]bool)
	for _, rt := range repoTags {
		part := fmt.Sprintf("('%s', '%s', TIMESTAMP WITH TIME ZONE '%s')", rt.OrgRepoName, rt.TagName, rt.Created.Format(time.RFC3339))
		repoTagsValueParts = append(repoTagsValueParts, part)
		orgRepoNames[rt.OrgRepoName] = true
	}
	for orgRepoName := range orgRepoNames {
		if orgRepoNamesConditionals == "" {
			orgRepoNamesConditionals += fmt.Sprintf("WHERE org_repo_name = '%s'", orgRepoName)
		} else {
			orgRepoNamesConditionals += fmt.Sprintf(" OR org_repo_name = '%s'", orgRepoName)
		}
	}
	...
	query := "DELETE FROM repo_tags " + orgRepoNamesConditionals
...
SET created = EXCLUDED.created;`, strings.Join(repoTagsValueParts, ",\n\t"))
	if _, err := tx.QueryContext(ctx, query); err != nil {
		return fmt.Errorf("StoreRepoTags:\nquery: %s\nerror: %v", query, err)
	}

	query = `UPDATE repos
SET indexing_finished = NOW() ` + orgRepoNamesConditionals + ";"
	if _, err := tx.ExecContext(ctx, query); err != nil {
		return fmt.Errorf("StoreRepoTags:\nquery: %s\nerror: %v", query, err)
	}
